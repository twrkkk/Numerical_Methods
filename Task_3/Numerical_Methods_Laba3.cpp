...
std::vector<std::vector<double>> generateMatrixWithLambda(vector<double> l)
{
	int N = l.size();
...
vector<tuple<double, double>> test(int n, int l_min, int l_max, double eps, int& max_iteration, double& max_aij)
{
	vector<double> expected_lambda(n);
	for (int i = 0; i < n; ++i)
		expected_lambda[i] = l_min + static_cast<double>(rand()) / RAND_MAX * (l_max - l_min);

	auto symmetricMatrix = generateMatrixWithLambda(expected_lambda);
...
int main() {
	...
	for (int c = 0; c < test_count; ++c)
	{
		...
		for (int i = 0; i < iteration; ++i)
		{
			...
			auto result = test(n, l_min, l_max, eps, max_iteration, max_aij);
